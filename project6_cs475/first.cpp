...
//Different global sizes to try. 1k to 8 million
int global_size[9] = {1000,2000,5000,10000,100000,1000000,5000000,8000000,8000000};
//Different local sizes to try. 1 - 1024
int local_size[11] = {1,2,4,8,50,100,200,500,800,1024,1024};
...
{
    for (int gSize = 0; gSize < 8; gSize++) {
        for (int lSize = 0; lSize < 10; lSize++) {
            ...
            float *hA = new float[ global_size[gSize] ];
            float *hB = new float[ global_size[gSize] ];
            float *hC = new float[ global_size[gSize] ];

            // fill the host memory buffers:

            for( int i = 0; i < global_size[gSize]; i++ )
            {
                hA[i] = hB[i] = (float) sqrt(  (double)i  );
            }

            size_t dataSize = global_size[gSize] * sizeof(float);
            ...
            size_t globalWorkSize[3] = { global_size[gSize], 1, 1 };
            size_t localWorkSize[3]  = { local_size[lSize],   1, 1 };
            ...
            //fprintf( stderr, "%8d\t%4d\t%10d\t%10.3lf GigaMultsPerSecond\n",
             //   global_size[gSize], local_size[lSize], NUM_WORK_GROUPS, (double)global_size[gSize]/(time1-time0)/1000000000. );
            std::cout << "Global Size: " << global_size[gSize]
            << " Local Size: " << local_size[lSize]
            << " Work Groups: " << global_size[gSize]/local_size[lSize]
            << " GigaMultsPerSecond: " << (double)global_size[gSize]/(time1-time0)/1000000000 << "\n";
        #ifdef WIN32
            Sleep( 2000 );
        #endif
